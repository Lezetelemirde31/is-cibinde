...
from app.schemas import EmployerAnalytics, EmployerApplicationRow, EmployerJobRow
...
@router.get("/analytics", response_model=EmployerAnalytics)
def analytics(user: User = Depends(require_roles("employer")), db: Session = DbDep):
    return dashboard_service.employer_analytics(db, user.id)
